...
def writeEV(date, dinger, parx=False, silent=False):
	...
	gameStarted = {}
	for gameData in schedule[date]:
		if not gameData["start"] or gameData["start"] == "LIVE" or gameData["start"] == "Suspended":
			gameStarted[gameData["game"]] = True
		else:
			dt = datetime.strptime(gameData["start"], "%I:%M %p")
			dt = int(dt.strftime("%H%M"))
	...
	for game in data:
		...
		for player in data[game]:
			...
			if "dk" in books and "++" not in data[game][player]["dk"]:
				...
				o20 = convertAmericanOdds(1 + (convertDecOdds(o) - 1) * 1.20)
				o34 = convertAmericanOdds(1 + (convertDecOdds(o) - 1) * 1.34)
				devig(evData, playerFinal, ou, o34, book="dk-34")
				...
				if "circa" in books:
					try:
						...
						devig(evData, playerFinal, data[game][player]["circa"], o20, book="dk-20-vs-circa")
						devig(evData, playerFinal, data[game][player]["circa"], o34, book="dk-34-vs-circa")
					except:
						pass
				pass
			if "cz" in books:
				o = int(data[game][player]["cz"].split("/")[0])
				o50 = convertAmericanOdds(1 + (convertDecOdds(o) - 1) * 1.50)
				devig(evData, playerFinal, ou, o50, book="cz-50")

				o = int(data[game][player]["cz"].split("/")[0])
				o30 = convertAmericanOdds(1 + (convertDecOdds(o) - 1) * 1.30)
				devig(evData, playerFinal, ou, o30, book="cz-30")

				if "circa" in books:
					try:
						devig(evData, playerFinal, data[game][player]["circa"], o30, book="cz-30-vs-circa")
						devig(evData, playerFinal, data[game][player]["circa"], o50, book="cz-50-vs-circa")
					except:
						pass
				pass
			if "espn" in books:
				...
				if "circa" in books:
					try:
						...
						devig(evData, playerFinal, data[game][player]["circa"], int(data[game][player]["espn"].split("/")[0]), book="espn-hr-vs-circa", promo="espn-hr")
					except:
						pass
			...
			if "fd" in books:
				...
				devig(evData, playerFinal, ou, fd, book="fd-sweat", promo="fd-sweat")
				devig(evData, playerFinal, ou, fd, book="fd")
				devig(evData, playerFinal, data[game][player]["fd"], highest, book="vs-fd")
				...
				if "circa" in books:
					try:
						devig(evData, playerFinal, data[game][player]["circa"], int(data[game][player]["fd"]), book="fd-sweat-vs-circa", promo="fd-sweat")
			...
			if playerFinal not in evData:
				continue
			elif evData[playerFinal].get("ev", 0) > 0 and not silent:
...
def writeLineups(date):
	...
	if date == "2025-07-12":
		data["min"]["pitcher"] = "travis adams"
		data["atl"]["pitcher"] = "nathan wiles"
		pass
