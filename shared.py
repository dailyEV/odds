...
def writeHistorical(date, book, gameStarted=None):
	...
	if today and not gameStarted:
		...
		for gameData in schedule[date]:
			if not gameData["start"] or gameData["start"] == "LIVE" or gameData["start"] == "Postponed" or gameData["start"] == "Suspended":
...
def parsePlayer(player):
	...
	# MLB
		...
		return "max muncy"
	elif player == "tan happ":
		return "ian happ"
	elif player == "matt nclain":
		return "matt mclain"
	...
	elif player == "jose ranirez":
		return "jose ramirez"
	elif player == "nookie betts":
		return "mookie betts"
	elif "machado" in player:
		return "manny machado"
	elif player == "andrew mccutcheon":
		return "andrew mccutchen"
	elif player == "vladinir guerrero":
		return "vladimir guerrero"
	elif player == "naikel garcia":
		return "maikel garcia"
	elif player == "jonah hein":
		return "jonah heim"
	elif player == "shohet ohtani":
		return "shohei ohtani"
	elif player == "niguel vargas":
		return "miguel vargas"
	...
	elif player.startswith("nark "):
		player = player.replace("nark ", "mark ")
	return player
